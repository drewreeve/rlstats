...
`file_outcome.encode()` / `decode()` are the sole owners of the `.replay.ingested` sentinel's on-disk string format — `"written"` or `"skipped:<reason>"` — and `sentinel_path()` owns the `.ingested` suffix. Empty sentinels, written before the written/skipped distinction existed, decode as `Written`; an unrecognized `skipped:<reason>` decodes to `Skipped(None)` rather than raising, so a newer writer can't break an older reader. `Failed` files get no sentinel, so they're retried on the next run.
