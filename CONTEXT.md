...
A **file outcome** is the per-replay-file result of the ingest pipeline, one of three variants: `Written` (the replay was analyzed and its match row written), `Skipped` (carrying a `SkipReason` — no match GUID, missing date, or no tracked players — from `validate_replay()`), or `Failed` (carrying an error message, from a corrupt replay or a DB write failure). `analyze_replay()` in `ingest.py` produces `Written | Skipped` (`AnalysisResult`); `write_parsed_batch()` in `process.py` produces the full `Written | Skipped | Failed` (`FileOutcome`) once it knows whether the DB write succeeded. The `.replay.ingested` sentinel file's *content* (not just its existence) records which of `Written`/`Skipped` applies — `"written"` or `"skipped:<reason>"` — so `UploadProcessor.status()` can report a genuine `skipped` state to the uploader instead of conflating it with `processed`. Sentinels written before this distinction existed are empty and are read as `Written`, preserving their original meaning. `Failed` files get no sentinel, so they're retried on the next run.
