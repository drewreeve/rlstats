...
The **replay wire** is the contract between `replay_frames.py` and the browser replay viewer (`static/replay-core.js`, `static/replay.js`): the shape of the `/api/matches/{id}/replay` metadata JSON and the `/api/matches/{id}/replay-frames.bin` position buffer. `replay_frames.py` is its sole owner.
